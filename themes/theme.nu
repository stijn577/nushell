...
# def _change_by_key [config_path: path, keys: list<string>, vals: list<string>]: nothing -> closure {
#   {|config_path keys vals|
...
#     let conf = open $config_path
#     for i in 0..(($keys | length) - 1) {
#       conf | update ($keys | get $i) ($vals | get $i)
#     }
#     $conf | save $config_path 
#   }
# }

# def _change_by_string [config_path: path, keys: list<string>, vals: list<string>]: nothing -> closure {
#   {|config_path keys vals| 
#     if not ($config_path | path exists) {
#       error make -u { msg: $"config path ($config_path) does not exist" }
#     }
