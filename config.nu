        ...
        PWD: [
            {
                ...
                code:{
                    if ("build.nu" | path exists) {
                        $env.SOURCE_BUILD_NU = true
                    } else {
                        $env.SOURCE_BUILD_NU = false
                    }
                }
            }
            {
                condition: {|_, _| $env.SOURCE_BUILD_NU }
                code: "echo \"Sourcing build.nu...\" ; source build.nu ; echo \"build.nu sourced!\""
            }
        ]
    }
})

# auto-source build.nu, with user confirmation
# $env.config = ($env.config | upsert hooks {
#     env_change: {
#         PWD: [
#             {
...
#                 code:{
#                     if ("build.nu" | path exists) {
#                         let user_input = (input "build.nu found! Do you want to source it? (y/n, default: y) ")
